...
import { formatCash, getRandomAvatar, time_ago } from "../../utils";
...
const HomeCard: React.FC<Props> = ({ active = false, meta = {
  ...
  bong_tai: "Cấp 1",
  moc_quay: "10k",
}, note = "Liên hệ ngay bằng cách ấn các nút bên dưới", price = 0, contact = {}, images, user, id, createdAt }) => {
      ...
      <div className="pt-2 overflow-hidden ">
        <div className="flex items-center px-2">
          ...
          <div className="flex items-center justify-between w-full">
            ...
            <div>
              <Menu button={<div className="p-1 flex items-center"><BsThreeDots /></div>} options={options} />
            </div>
          </div>
        </div>
        ...
        <div className="flex border-t border-divide items-center  overflow-hidden">
          ...
          {contact?.phone && <div onClick={() => {
            window.open(`tel:${contact?.phone}`, "_blank");
          }} className=" flex-1 flex items-center hover:opacity-70 transition-all  justify-center gap-1 text-sm text-center hover:cursor-pointer py-2">
            <MdLocalPhone />
            <span>Điện thoại</span>
          </div>}
          {contact?.messenger && <div onClick={() => {
