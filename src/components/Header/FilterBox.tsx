            ...
            >
                <div className='h-full bg-dark1 py-4 px-4'>
                    <div className='flex items-center justify-between pb-4 border-b border-divide'>
                        ...
                        <button className='underline text-sm hover:text-primary transition-all'>Xoá tất cả</button>
                    </div>
                    <div className='mt-4'>
                        <Dropdown title="Hành tinh">
                            <div className='mt-4 space-y-2'>
                                {planes.map((item, index) => {
                                    return <div key={item.value} className='flex items-center gap-4 hover:cursor-pointer hover:text-primary ' onClick={() => handleToggle(item.value)}>
                                        <Checkbox checked={filter?.planet?.includes(item.value)}
                                        />
                                        <div className='items-center flex gap-1'>
                                            <LazyLoadImage src={item.icon} effect='blur' className='w-7' />
